...
export class InteractiveObject {
    ...
    actionTarget: number;
    actions: {key: string, sound: Sound, object: FloatingText, options: string[]}[];


    constructor (spriteSrc: string, size: number, initialPos: position, animationPeriod: number, states: number, actions: iconKeyAndTexts[]) {
    ...
    private renderTexts(canvas: CanvasRenderingContext2D){
        ...
        const action = (this.currentState === 0)
        ? this.actions[0].options[0]
        : this.actions[0].options[1];
        this.actions[0].object.setText(action);

        this.actions.forEach((action, i) => {
            if((i === 0) || ((this.currentState > 0) && (this.currentState < (this.states - 1)))){                
    ...
    update(keyPressed: string | undefined, players: Player[], callback: () => void){
