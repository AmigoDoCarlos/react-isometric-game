...
export class Sprite {
    ...
    constructor (sprite: string, size: number, rows: number, columns: number, maxCount: number) {
    ...
    render(canvas: CanvasRenderingContext2D, position: position){    
        ...
        canvas.drawImage(
            ...
            position.x, position.y,                     // coordenadas X e Y do início do desenho (canto superior esquerdo)
            this.size, (this.size * ratio),             // tamanho final da imagem (width, height)
        );
    }
}